...
app.get('/api/health', async (req, res) => {
  let ytmusicStatus = 'not initialized';
  
  if (ytmusicReady && ytmusic) {
    try {
      // Try a quick test search
      const testResults = await ytmusic.searchSongs('test');
      ytmusicStatus = `working (test returned ${testResults?.length || 0} results)`;
    } catch (err) {
      ytmusicStatus = `error: ${err.message}`;
    }
  } else if (ytmusic) {
    ytmusicStatus = 'initialized but not ready';
  }
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    console.log(`Found ${results.length} raw results for: ${q}`);
    
    // Keywords that indicate non-studio versions to filter out
    ...
    // Keywords that indicate preferred versions (boost these)
    ...
    let songs = results.slice(0, 30).map(song => {
      // Handle potential null/undefined song objects
      if (!song) return null;
      
      const title = song.name || song.title || '';
      const albumName = song.album?.name || '';
      const fullText = `${title} ${albumName}`;
      
      // Check if it matches exclude patterns
      const isExcluded = excludePatterns.some(pattern => pattern.test(fullText));
      
      // Check if it matches preferred patterns (gets priority)
      const isPreferred = preferPatterns.some(pattern => pattern.test(fullText));
      ...
      return {
        ...
        thumbnail: song.thumbnails?.[0]?.url || song.thumbnail || null,
        explicit: song.isExplicit || false,
        album: albumName,
        _excluded: isExcluded,
        _preferred: isPreferred
      };
    }).filter(song => song !== null && song.id); // Remove null entries and entries without ID
    
    // Filter out excluded versions (but keep at least some results)
    const filteredSongs = songs.filter(song => !song._excluded);
    
    // If filtering removed everything, use unfiltered results
    if (filteredSongs.length === 0 && songs.length > 0) {
      console.log('All results were filtered out, using unfiltered results');
    } else {
      songs = filteredSongs;
    }
    
    // Sort: preferred versions first
    songs.sort((a, b) => {
    ...
    songs = songs.map(({ _excluded, _preferred, ...song }) => song);
