...
def run_ffplay(args, **kwargs):
    """Run ffplay with Windows compatibility"""
    cmd = ['ffplay'] + args
    # On Windows, we need shell=False but proper PATH handling
    # subprocess.Popen handles this better than run for long-running processes
...
def download_from_youtube(song_id, title, artist):
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=120, shell=(sys.platform == 'win32'))
...
def start_crossfade_song(file_path, volume):
    try:
        ...
        process = run_ffplay(['-nodisp', '-autoexit', '-af', audio_filter, file_path])
...
def play_song_with_crossfade(file_path, next_file, queue_id, is_auto=False):
    try:
        ...
        with player_state.lock:
            player_state.current_process = run_ffplay(['-nodisp', '-autoexit', '-af', audio_filter, file_path])
...
def resume_song(file_path, start_position, queue_id):
    try:
        ...
        with player_state.lock:
            player_state.current_process = run_ffplay(['-nodisp', '-autoexit', '-ss', str(start_position), '-af', audio_filter, file_path])
...
def get_auto_playlist_song():
    ...
    try:
        response = requests.get(f"{SERVER_URL}/api/library/random", timeout=5)
        if response.ok:
            data = response.json()
        ...
        try:
            ...
            if songs:
                ...
                if auto_song and auto_song.get('file_path'):
                    ...
                    # Get another random song for crossfade
                    next_auto = get_auto_playlist_song()
                    next_file = next_auto.get('file_path') if next_auto else None
                    
                    result = play_song_with_crossfade(file_path, next_file, None, is_auto=True)
                    # Don't mark auto-playlist songs as "played" in queue
                else:
                    with player_state.lock:
...
if __name__ == '__main__':
    ...
    for cmd, name in [('yt-dlp', 'yt-dlp'), ('ffplay', 'ffmpeg'), ('ffprobe', 'ffprobe')]:
        try:
            # Use shell=True on Windows to properly find executables in PATH
            ...
            print(f"   Make sure {cmd} is installed and in your PATH")
            print(f"   Try running '{cmd} -version' in a new terminal to verify")
            sys.exit(1)
